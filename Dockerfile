...
RUN unzip riot-stream-2.15.4.zip && \
    rm riot-stream-2.15.4.zip && \
    chmod +x riot-stream-2.15.4/bin/riot-stream


ENV PATH="${PATH}:/riot-stream-2.15.4/bin"

ENTRYPOINT ["/riot-stream-2.15.4/bin/riot-stream"]
